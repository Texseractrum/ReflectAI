...
fastify.addHook("onRequest", async (request, reply) => {
    ...
    try {
        ...
        if (username !== "admin" || password !== "reflectai123") {
            ...
            const setupElevenLabs = async () => {
                try {
                    ...
                    elevenLabsWs.on("open", () => {
                        console.log(
                            "[ElevenLabs] Connected to Conversational AI"
                        );
                        logEvent(
                            callSid,
                            "[ElevenLabs]",
                            "Connected to Conversational AI"
                        );
                    ...
                    switch (msg.event) {
                        ...
                        case "stop":
                            ...
                            if (callSid && callLogs[callSid]) {
                                ...
                                console.log(
                                    `[Server] Conversation saved for CallSid ${callSid}`
                                );
                            }
                        ...
                        default:
                            console.log(
                                `[Twilio] Unhandled event: ${msg.event}`
                            );
                            logEvent(
                                callSid,
                                "[Twilio]",
            ...
            ws.on("close", () => {
                ...
                Return ONLY the raw JSON in this exact structure NOT INCLUDING ANY MARKDOWN FORMATTING OR CODE BLOCKS:
                {
                  ...
                  "summary": string
                            ...
                            console.log(
                                `[Server] Summary for CallSid ${callSid} written to ${summaryFile}`
                            );
                        } catch (error) {
                            console.error(
...
fastify.get("/analyze-conversation/:conversationId", async (request, reply) => {
    try {
        ...
        try {
            ...
            return reply.type("application/json").send(parsedData);
        } catch (parseError) {
            console.error("Error parsing summary data:", parseError);
            console.error("Summary data that failed to parse:", summaryData);
